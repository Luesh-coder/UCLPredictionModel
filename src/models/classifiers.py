...
class MarketBaseline(BaseEstimator, ClassifierMixin):
    """Replays the bookmaker's own de-vigged probabilities as a forecast.

    This is the benchmark, and it is a punishing one. Closing odds aggregate the
    money of everyone with an opinion, including people with team news, injury
    information and models better than this one. Approaching it is the realistic
    Stage 1 success condition; beating it materially, on a first pass, over
    thousands of matches, is much more likely to mean a feature has leaked than
    that the market has been solved.

    Unlike every other model here it has nothing to learn — `fit` exists only to
    satisfy the estimator interface, and the columns it reads were computed by
    `src.ingest.odds` at ingestion time.
    """

    def __init__(self, columns: tuple[str, str, str] = ("odds_H", "odds_D", "odds_A")):
    ...
    def predict_proba(self, X):
        ...
        # valid distribution for every row it is handed. The evaluation layer
        # reports how many rows this affected.
        blank = ~np.isfinite(proba).all(axis=1)
        if blank.any():
            proba[blank] = _MARKET_FALLBACK
        return proba / proba.sum(axis=1, keepdims=True)


# Long-run big-five base rates, used only for matches no bookmaker priced.
_MARKET_FALLBACK = np.array([0.45, 0.25, 0.30])
...
def make_lightgbm(features: list[str] | None = None, *, calibrate: bool = True, **params):
    ...
    Defaults are deliberately small. Football results are close to
    irreducibly noisy — even a perfect model tops out somewhere near the
    bookmaker's accuracy — so the ceiling here is set by the sport, not by
    capacity, and an unconstrained LightGBM spends its extra capacity
    memorising noise and emitting over-confident probabilities that log loss
    punishes hard. `calibrate=True` wraps the model in isotonic calibration
    fitted on held-out folds, which usually buys more than any hyperparameter
    tweak.
    """
